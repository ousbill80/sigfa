 ...
 * hex. `deriveBankTheme` turns that single value into the four `--brand*`
 * tokens the design system consumes, keeping the SIGFA STRUCTURE and the fixed
 * functional palette (`--forest` / `--gold` / semantics) untouched.
 *
...
/**
 * Derive the four `--brand*` tokens from a single tenant brand hex.
 *
...
export function deriveBankTheme(brandHex: string): BankTheme {
  ...
  return { brand, brandStrong, brandSoft, brandContrast };
}
