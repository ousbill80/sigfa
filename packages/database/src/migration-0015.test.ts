 ...
 *   - [OK] tenant A PEUT `UPDATE ... SET <3 seuils>, updated_at = NOW()` sur SA ligne ;
 *   - [NON] le même UPDATE sur la ligne du tenant B → 0 ligne (RLS tenant_update) ;
 *   - [NON] UPDATE d'une AUTRE colonne (name) → permission denied (hors GRANT) ;
