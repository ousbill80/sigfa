...
| CONTRACT-010 | Corrections panel : durcissement sécurité + cohérence inter-YAML + exemples UUID | agent-contract | 001–009 | DONE |
...
- Lint : **spectral** zéro erreur (ruleset commité `packages/contracts/.spectral.yaml`, créé par 001) incluant les **règles custom** : `x-required-role` ∈ {SUPER_ADMIN, BANK_ADMIN, AGENCY_DIRECTOR, MANAGER, AGENT, AUDITOR, AUTHENTICATED (JWT requis sans rôle minimal — ajouté par CONTRACT-010), NONE (aucune auth)}, `x-tenant-scope` ∈ {platform, bank, agency, public} — obligatoires sur chaque opération.
