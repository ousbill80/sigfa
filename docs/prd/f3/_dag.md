...
1. **Panel Boucle 3 FAIT** (`30-boucle3-panel-f3-arbitrage.md`) — 1 BLOCKER (escalade AUDITOR→AGENT) + 8 findings CORRIGÉS (commit `4312c83` : RBAC AUDITOR lecture seule, heartbeat cross-tenant, escalade import CSV, idempotence POST /tickets atomique, révocation borne sur WS, trust-proxy XFF, algos JWT, scope kiosks/status, test de course renforcé). **CONSIGNÉ au durcissement** : SEC-F3-02 (recâbler l'accès DB via `withTenant`/RLS armée — l'isolation repose aujourd'hui sur le `WHERE bank_id` applicatif testé), COV-01 (activer Schemathesis `response-schema-conformance`), STY DRY (`@sigfa/schemas` non importé, types generated non consommés, `errorResponse` dupliqué, harnais de test dans `src/`), + MINORs.
