...
async function runMigrations(client: pg.Client): Promise<void> {
  ...
  await client.query(`CREATE TABLE IF NOT EXISTS users (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), bank_id UUID REFERENCES banks(id), email TEXT NOT NULL UNIQUE, first_name TEXT NOT NULL DEFAULT 'A', last_name TEXT NOT NULL DEFAULT 'B', role TEXT NOT NULL DEFAULT 'AGENT', languages agent_language[] NOT NULL DEFAULT '{FR}', work_schedule JSONB, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), is_relationship_manager BOOLEAN NOT NULL DEFAULT false, display_name TEXT, photo_url TEXT
);`);
