...
export default defineConfig({
  resolve: {
    alias: {
      ...
      // CONTRACT-014 : l'alias @sigfa/contracts/events/realtime.js est supprimé —
      // le package expose désormais `"./events/*": "./dist/events/*"` dans ses
      // exports, la résolution Node standard suffit (dist du contrat construit).
  ...
  test: {
    ...
    coverage: {
      ...
      exclude: [
        ...
        "src/audit/audit-log-test-schema.ts", // Support de test partagé SEC-001a (schéma audit_log Testcontainers)
        "src/routes/tickets-test-migrations.ts", // Support de test partagé cluster tickets (applyMigrations Testcontainers)
      ],
    },
  },
});
