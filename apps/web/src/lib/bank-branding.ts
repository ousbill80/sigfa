/**
 * bank-branding — identité d'affichage de la banque côté web (WEB-002-HDR).
 *
 * Même convention de provisionnement que apps/kiosk `src/lib/kiosk-branding.ts`
 * (theming banque sans effort) : `NEXT_PUBLIC_BANK_NAME` +
 * `NEXT_PUBLIC_BANK_LOGO_URL`, avec replis sûrs — sans logo, l'UI retombe sur
 * la pastille `--brand` + initiale ; aucun asset n'est jamais requis pour
 * brander une banque.
 *
 * NOTE theming : le branchement sur l'endpoint theme tenant
 * (`GET /public/banks/{id}/theme`) remplacera ces envs — chantier theming
 * parallèle en cours ; l'API de ce module (nom + logo + initiale) est la
 * couture prévue pour cette bascule.
